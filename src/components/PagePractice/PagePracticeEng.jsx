...
const PagePractice = observer(() => {
   ...
   useEffect(() => {
      fetchWords().then(() => setLoading(false)); // Выключаем индикатор loading
   }, [fetchWords]);
   
   // for rendering words
   const [currentIndex, setCurrentIndex] = useState(0);
   useEffect(() => {
      if (words.length > 0) { // Добавлена проверка, т.к. когда words пустые (до загрузки), currentIndex = 0 может указывать на undefined, что вызывает ошибку рендеринга
         setCurrentIndex(0); // Сбрасываем currentIndex на 0, когда слова загружены
      }
   }, [words]);
   ...
   const ref = useRef();
   useEffect(() => {
      if (ref.current) { // ref.current может быть undefined, если кнопка check ещё не отрендерилась, поэтому необходимо добавлять проверку if
         ref.current.focus();
      }
   }, [currentIndex]); // useEffect срабатывает каждый раз, когда currentIndex изменяется, то есть при смене слова, т.о. фокус на кнопке check устанавливается не один раз (при самом первом рендере), а каждый раз при смене currentIndex(смене слова)
